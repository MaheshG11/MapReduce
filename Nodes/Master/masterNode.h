...
class MasterServer : public NodeServer{
 private:
  ...
  int32_t workers,mappers,reducers,completed,working;
  ...
  bool executing;
  void BroadcastFilePathToReducers(const ::FileInfo* request);
 public:
  MasterServer(std::string &folderPath,std::string &ipPort);
  ~MasterServer() override;
  grpc::Status sendNodeInfo(::grpc::ClientContext* context, const ::NodeInfo* request, ::isReceived* response);// Used For registering/subscribing workers with master
  grpc::Status sendFileInfo(::grpc::ClientContext* context, const ::FileInfo* request, ::isReceived* response);// Used to relay file locations on mappers to reducers
  grpc::Status sendSignal(::grpc::ClientContext* context, const ::isReceived* request, ::isReceived* response);// Used to recieve finished signal
  int32_t getWorkers();
