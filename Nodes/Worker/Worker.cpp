...
void WorkerServer::keepBusy(){
    while(true){
        ...
        */
        ...
        isBusy_lock.unlock();
        node_client.getFile(0,filePath,"","",this->node_client.getStub(ipPort),filePath);
        file_lock.lock();
...
const ::isReceived* request, ::isReceived* response){
    ...
    if(type>0){
        ...
        node_client.getFile((int64_t)0,std::string("mapper.cpp"),std::string(""),std::string(""),node_client.master_stub,
        filePath_);
        ...
        while(true){
            (this->file_lock).lock();
            if((this->completed)==2 and (this->fileBuffer).size()){
                this->file_lock.unlock();
                break;
            }
            file_cv.wait(file_lock,[this]{return fileBuffer.size()>zero;});
            std::string path=(this->fileBuffer).back();
            fileBuffer.pop_back();
            file_lock.unlock();
...
grpc::Status WorkerServer::sendFileInfo(::grpc::ClientContext* context, const ::FileInfo* request, ::isReceived* response){
    ...
    (this->fileInfoBuffer).push_back({filePath+std::to_string(this->r_id)+".txt",ipPort});
